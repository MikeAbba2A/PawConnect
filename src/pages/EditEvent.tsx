...
import { getEventById, updateEvent, getEventTypes, uploadEventImage } from '../lib/eventService';
...
const EditEvent = () => {
  ...
  const loadEventData = async () => {
    ...
    try {
      ...
      const endDate = eventData.date_end ? new Date(eventData.date_end) : null;
      ...
      setFormData({
        ...
        time_start: startDate.toTimeString().slice(0, 5),
        date_end: endDate ? endDate.toISOString().split('T')[0] : '',
        time_end: endDate ? endDate.toTimeString().slice(0, 5) : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update event
      const { event: updatedEvent, error: updateError } = await updateEvent(event.id, {
    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-red-50 border border-red-200 text-red-600 p-4 rounded-lg">
          Événement non trouvé ou vous n'avez pas les permissions pour le modifier.
        </div>
      </div>
    );
  }
